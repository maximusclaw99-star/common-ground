...
| **Tool calling on Foundation Model APIs** | `POST /serving-endpoints/chat/completions` with `tools` on `databricks-meta-llama-3-3-70b-instruct` returns proper `tool_calls` in ~1 s on Free Edition. This is what the plan agent runs on; no custom Model Serving needed. |
...
| Feature | Result |
|---|---|
| **Outbound HTTP from the warehouse** (`http_request()` / UC HTTP connections) | The function runs, but Free Edition's serverless network policy blocks every external host (`Access to httpbin.org is denied because of serverless network policy`) and there is no account console to allow-list one. A serverless *notebook job* can reach `rapidapi.com` and `api.github.com` but not `media.licdn.com`. Probed 2026-09-19. |
